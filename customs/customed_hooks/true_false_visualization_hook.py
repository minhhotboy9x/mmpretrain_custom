...
class TFVisualizationHook(Hook):
    ...
    def _draw_samples(self,
        ...
        for sample_id in range(first_sample_id, end_idx, self.interval):
            ...
            if self.out_dir is not None:
                gt_id = data_sample.gt_label.item()
                pred_id = data_sample.pred_label.item()
                if self.class_names is not None:
                    gt_name = self.class_names[gt_id]
                    pred_name = self.class_names[pred_id]
                    draw_args['out_file'] = join_path(self.out_dir,
                                                      gt_name, pred_name,
                                                        f'{batch_idx}_{sample_id}_{sample_name}')
                else:
                    draw_args['out_file'] = join_path(self.out_dir,
                                                      gt_id, pred_id,
                                                        f'{batch_idx}_{sample_id}_{sample_name}')
