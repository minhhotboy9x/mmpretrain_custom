...
# ----------------- Training ----------------- #
def train(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, log_dir="work_dirs/gen_image_logs"):
    ...
    for epoch in range(num_epochs):
        ...
        for step, (images, labels) in enumerate(val_loop): 
            with torch.no_grad():
                model.eval()
                sample_img, _ = val_loader.dataset.sample_minor_img(val_loader.batch_size)
                sample_img = sample_img.to(device)
                sample_recon = model(sample_img)
                val_loss += criterion(sample_recon, sample_img)
            
            save_image(sample_img[0].cpu(), os.path.join(log_dir, f"original_batch_{0}.png"))
            save_image(sample_recon[0].cpu(), os.path.join(log_dir, f"reconstructed_batch_{0}.png"))
            save_image(sample_img[-1].cpu(), os.path.join(log_dir, f"original_batch_{1}.png"))
            save_image(sample_recon[-1].cpu(), os.path.join(log_dir, f"reconstructed_batch_{1}.png"))
